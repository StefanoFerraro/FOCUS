...
def process_frame(deva: DEVAInferenceCore,
    ...
    h, w = image_np.shape[:2]
    new_min_side = cfg['size']
    image = get_input_frame_for_deva(image_np, new_min_side).detach()

    frame_name = "frame_img"
    frame_info = FrameInfo(image, None, None, first, {
        'frame': [frame_name],
        'shape': [h, w],
    })
    ...
    if first:
        ...
        # Run the model on this frame
        image_np = torch.Tensor(image_np.copy().transpose(2, 0, 1)).float().to("cuda:0")
        prob = deva.step(image_np, None, None)
    return prob