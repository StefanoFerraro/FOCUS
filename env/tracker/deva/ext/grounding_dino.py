...
def segment_with_text(config: Dict, gd_model: GroundingDINOModel, sam: SamPredictor,
    ...
    boxes, logits, phrases = predict(
        ...
        caption=prompts[0], # TODO check that the full string is passed and
        box_threshold=BOX_THRESHOLD,
        text_threshold=TEXT_THRESHOLD,
        device=gd_model.device,
    )
    
    source_h, source_w, _ = image.shape
    detections = gd_model.post_process_result(
            source_h=source_h,
            source_w=source_w,
            boxes=boxes,
            logits=logits)
    class_id = gd_model.phrases2classes(phrases=phrases, classes=classes)
